...
static int yajl_start_map_cb(void *a_ctx)
{
        ...
        if(l_prefix_len)
        {
                ...
                // -----------------------------------------
                // strncat appends null character at end,
                // -1 to account
                // -----------------------------------------
...
static int yajl_end_map_cb(void *a_ctx)
{
        ...
        if(l_sep)
        {
                strncpy((char*)l_parser->m_prefix, (char *)(l_sep - (char *)l_parser->m_prefix), PARSER_JSON_PREFIX_LEN_MAX-1);
                //NDBG_PRINT("l_parser->m_prefix: %s\n", l_parser->m_prefix);
                strncpy((char*)l_parser->m_current_key, l_sep + 1, PARSER_JSON_PREFIX_LEN_MAX-1);
        }
        else
        {
