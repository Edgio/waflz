...
{
        if(!ao_enf.has_enf_type() &&
            ao_enf.has_type())
        {
                const std::string &l_type = ao_enf.type();
#define _ELIF_TYPE(_str, _type) else \
if(strncasecmp(l_type.c_str(), _str, sizeof(_str)) == 0) { \
        ao_enf.set_enf_type(waflz_pb::enforcement_type_t_##_type); \
}
            if(0) {}
        ...
        // -----------------------------------------
        if(!ao_enf.has_response_body() &&
           ao_enf.has_response_body_base64() &&
           !ao_enf.response_body_base64().empty())
        {
            const std::string& l_b64 = ao_enf.response_body_base64();
            char* l_body = NULL;
            size_t l_body_len = 0;
            int32_t l_s;
            ...
            ao_enf.mutable_response_body()->assign(l_body, l_body_len);
