...
//! @param   TODO
//! ----------------------------------------------------------------------------
static int32_t rr_parse_resp(tp_fields_list_t &ao_fields_list, const char *a_buf, size_t a_len)
{
...
//! @param   TODO
//! ----------------------------------------------------------------------------
static int32_t rr_render_resp(char* ao_buf,
                         size_t& ao_len,
...
//! @param   TODO
//! ----------------------------------------------------------------------------
int32_t render_resp(char** ao_buf,
               size_t& ao_len,
               const char *a_buf,
               size_t a_len,
               resp_ctx *a_ctx)
{
        ...
        *ao_buf = NULL;
        ...
        l_s = rr_parse_resp(l_tp_fl, a_buf, a_len);
        ...
        // -------------------------------------------------
        size_t l_len = 0;
        l_s = rr_render_resp(NULL, l_len, l_tp_fl, a_ctx);
        if(l_s != WAFLZ_STATUS_OK)
        {
                return WAFLZ_STATUS_ERROR;
        }
        ...
        // render
        // -------------------------------------------------
        l_s = rr_render_resp(l_buf, l_len, l_tp_fl, a_ctx);
        if(l_s != WAFLZ_STATUS_OK)
        {
...
//! \return  0: success
//! \param   a_resp_ctx: resp_ctx object
//! ----------------------------------------------------------------------------
extern "C" int32_t plugin_render_resp(char** ao_buf,
                                size_t* ao_len,
                                const char *a_buf,
                                size_t a_len,
                                resp_ctx *a_resp_ctx)
{
        return render_resp(ao_buf, *ao_len, a_buf, a_len, a_resp_ctx);
}
}
