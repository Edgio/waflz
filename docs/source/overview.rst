...
* **IP Tree**: We've had an internal `IP Tree <https://github.com/VerizonDigital/waflz/blob/master/src/op/nms.h>`_ kicking around our internal repos, that's performed well for us and seems to be faster than the `msc_tree <https://github.com/SpiderLabs/ModSecurity/blob/v2/master/apache2/msc_tree.h>`_ in the standard implementation -see: :ref:`appendix-iptrie`.  It's reusable as well outside of our library.
